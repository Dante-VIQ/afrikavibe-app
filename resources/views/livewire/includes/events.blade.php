        <div>
            ...
            <div class="relative mr-4 float-right" x-data="{ show: false }">
                <button x-on:click.prevent="show = true"><i class="fa fa-ellipsis text-3xl"></i></button>

                <div class="absolute right-1 h-auto w-24 bg-primary" x-show="show"
                    x-on:click.outside.prevent="show = false">
                    <ol class=" bg-slate-800 text-slate-300 justify-center p-2">
                        @can('update', $feature)
                            <div x-data="{ show: false }" x-cloak>
                                <li wire:click="edit({{ $feature->id }})"><a href="#"
                                        x-on:click.prevent="show = true">Edit</a></li>
                                ...
                                <div x-show="show" x-on:click.outside.prevent="show = false">
                                    @include('livewire.includes.feature-update')
                                </div>

                            </div>
                        @endcan
                        @can('delete', $feature)
                            <li wire:click="delete({{ $feature->id }})"><a href="#">Delete Post</a></li>
                        @endcan
                    </ol>
                </div>
            ...
            <div class="relative">
                ...
                <div class="bg-slate-400  w-full bg:blur-lg">
                    <a href="#" class="absolute text-lg text-black p-3 font-semibold ">{{ $feature->title }}</a>
                    {{-- <p class="p-4 italic">by Royal Media.</p> --}}
                </div>

                <img src="{{ asset('storage/' . $feature->image) }}" class="w-full h-80 rounded-xl bg-cover "
                    alt="">
                <!-- <h5 class="text-xl leading-tight text-gray-950 p-2">Milking millions from farming</h5> -->

                <ul class="justify-between align-content-center space-x-4 -my-44 pl-5 float-right">
                    <li>
                        <a href="{{ $feature->links }}"
                            ...
                            <i class="fa fa-message text-2xl" fill="current"></i>
                        </a>
                    </li>
                    <li>
