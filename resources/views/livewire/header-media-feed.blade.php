                    ...
                    @foreach ($this->headerMedia as $media)
                        <div class="w-full flex-shrink-0" wire:key="media-{{ $media->id }}">
                            @if ($media->media_type === 'image')
                                @if ($media->media_path)
                                    <img class="img-fluid h-96 w-full" src="{{ asset($media->media_path) }}" alt="{{ $media->title }}" />
                                @endif
            ...
            <template x-if="totalSlides > 1">
                <div>
                    ...
                    <button @click="previous()"
                        class="absolute left-4 top-1/2 transform -translate-y-1/2 z-30 bg-black/30 text-white p-3 rounded-full hover:bg-black/50 transition-colors">
                    ...
                    {{-- Next Button --}}
                    <button @click="next()"
                        class="absolute right-4 top-1/2 transform -translate-y-1/2 z-30 bg-black/30 text-white p-3 rounded-full hover:bg-black/50 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
